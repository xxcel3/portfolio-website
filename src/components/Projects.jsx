  ...
  const renderCard = (proj) => {
    const content = (
      <>
        ...
        {proj.imageSrc && <img src={proj.imageSrc} alt={proj.title} className="w-full h-64 object-cover rounded-lg mb-4" />}
    ...
    if (proj.link) {
      return (
        <a key={proj.title} href={proj.link} target="_blank" rel="noopener noreferrer" {...wrapperProps}>
          {content}
        </a>
      );
    }
    if (proj.videoSrc) {
      return (
        <div key={proj.title} {...wrapperProps} onClick={() => setSelectedVideo(proj.videoSrc)}>
          {content}
        </div>
      );
    }
    ...
    <>
      ...
      <section id="projects" className="w-full py-16 bg-transparent scroll-mt-20">
        <div className="max-w-screen-xl mx-auto px-8">
          <img src="/header.png" alt="Projects Header" className="mx-auto mb-8 w-78"/>
